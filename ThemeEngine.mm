...
static void WTLoadTheme(void) {
    ...
    WTThemeMeta = [config mutableCopy];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *name = self.themes[indexPath.row];
    [name writeToFile:WTSelectedThemeFile() atomically:YES encoding:NSUTF8StringEncoding error:nil];
    WTLoadTheme();
    [tableView reloadData];
    WTShowMessage([NSString stringWithFormat:@"Applied theme: %@", name]);
}
